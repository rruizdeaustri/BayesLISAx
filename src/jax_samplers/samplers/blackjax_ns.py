...
class BlackJAXDynamicNSS(BlackJAXNestedSampler):
    ...
    def run(self, key: PRNGKey | None = None) -> SamplerResult:
        ...
        weights = np.asarray(getattr(dyn, "merged", {}).get("weights")) if hasattr(getattr(dyn, "merged", {}), "get") and getattr(dyn, "merged", {}).get("weights") is not None else None
        parts = parts_u
        p_active_min = getattr(self.problem, "p_active_min", 1e-3)
        if hasattr(self.problem, "decode_batch"):
            try:
        ...
        try:
            ...
            if merged is not None:
                if getattr(merged, "samples", None) is not None:
                    diags["dynamic_merged_samples"] = int(np.asarray(merged.samples).shape[0])
                if getattr(merged, "weights", None) is not None:
                    ww = np.asarray(merged.weights)
